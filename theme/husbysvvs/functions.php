<?php
/**
 * Husbys VVS AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Husbys VVS AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through hu_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'husbysvvs', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'husbysvvs' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'husbysvvs' ) ) );

		register_block_pattern_category( 'husbysvvs-sections', array( 'label' => __( 'Husbys VVS AB sections', 'husbysvvs' ) ) );
		register_block_pattern_category( 'husbysvvs-pages', array( 'label' => __( 'Husbys VVS AB page layouts', 'husbysvvs' ) ) );
	}
);
