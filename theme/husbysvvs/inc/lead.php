 ...
 * or photo. They call hu_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Husbys VVS AB.
 *
 ...
 * @return string
 */
function hu_sample_note() {
	return hu_lead( 'ar_riktig' ) ? '' : ' <span class="hu-sample">(platshållare)</span>';
}
 ...
 * @return string File name for hu_lead_img().
 */
function hu_lead_bild( $i ) {
	$bilder = (array) hu_lead( 'bilder', array() );
 ...
 * @return string
 */
function hu_ring_label() {
	$namn = hu_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, hu_lead( 'telefon' ) ) : 'Ring ' . hu_lead( 'telefon' );
}
