...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'husbysvvs-leads' ),
					'singular_name' => __( 'Lead', 'husbysvvs-leads' ),
					'edit_item'     => __( 'Förfrågan', 'husbysvvs-leads' ),
					'search_items'  => __( 'Search leads', 'husbysvvs-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'husbysvvs-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'husbysvvs-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'husbysvvs-leads' ),
			'npl_phone'    => __( 'Phone', 'husbysvvs-leads' ),
			'npl_service'  => __( 'Needs', 'husbysvvs-leads' ),
			'npl_timeline' => __( 'When', 'husbysvvs-leads' ),
			'npl_zip'      => __( 'ZIP', 'husbysvvs-leads' ),
			'npl_source'   => __( 'Source', 'husbysvvs-leads' ),
			'npl_status'   => __( 'Status', 'husbysvvs-leads' ),
			'date'         => __( 'Received', 'husbysvvs-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'husbysvvs-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'husbysvvs-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'husbysvvs-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'husbysvvs-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'husbysvvs-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'husbysvvs-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'husbysvvs-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'husbysvvs-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'husbysvvs-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'husbysvvs-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'husbysvvs-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'husbysvvs-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
