...
add_filter(
	...
	function ( $exporters ) {
		$exporters['husbysvvs-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'husbysvvs-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'husbysvvs-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'husbysvvs-leads',
						'group_label' => __( 'Quote requests', 'husbysvvs-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['husbysvvs-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'husbysvvs-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
